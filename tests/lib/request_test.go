...
func TestBodyParser(t *testing.T) {
	...
	app.Post("/validate", func(c *fiber.Ctx) error {
		data := new(sample)
		return lib.ErrorBadRequest(c, lib.BodyParser(c, data))
	})

	res, body, err := lib.PostTest(app, "/validate", nil, "")
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, false, nil == body)
	utils.AssertEqual(t, 400, res.StatusCode)

	res, body, err = lib.PostTest(app, "/validate", nil, `{"name":"john"}`)
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, 400, res.StatusCode)
}
