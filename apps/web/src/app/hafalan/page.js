...
const STATUS_OPTIONS = [
    { value: 'not_started', label: 'Belum', icon: <BsCircle />, color: 'text-gray-400' },
    { value: 'in_progress', label: 'Proses', icon: <BsRecordCircle />, color: 'text-yellow-500' },
    { value: 'memorized', label: 'Hafal', icon: <BsCheckCircleFill />, color: 'text-emerald-500' },
];
...
const HafalanPage = () => {
    ...
    const load = async () => {
        try {
            ...
            setError('Gagal memuat data hafalan. Coba muat ulang halaman.');
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <Section>
                <div className='container mx-auto px-4 max-w-3xl'>
                    ...
                    <p className='text-sm text-gray-500 dark:text-gray-400 mb-6'>
                        Catat progress hafalan surah Al-Quran kamu
                    </p>
                    ...
                    {summary && (
                        <div className='grid grid-cols-3 gap-3 mb-6'>
                            <div className='bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-xl p-4 text-center'>
                                ...
                                <p className='text-xs text-gray-500 dark:text-gray-400 mt-1'>Hafal</p>
                            </div>
                            <div className='bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-100 dark:border-yellow-800 rounded-xl p-4 text-center'>
                                ...
                                <p className='text-xs text-gray-500 dark:text-gray-400 mt-1'>Proses</p>
                            </div>
                            <div className='bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-xl p-4 text-center'>
                                ...
                                <p className='text-xs text-gray-500 dark:text-gray-400 mt-1'>Belum</p>
                            </div>
                        </div>
                    )}
                        ...
                        {hafalan.map((surah) => (
                            ...
                            >
                                ...
                                <div className='flex items-center gap-2'>
                                    <StatusBadge status={surah.status ?? 'not_started'} />
                                    <select
