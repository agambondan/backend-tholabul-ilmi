...
// LoadEnvironmentLocalFlag Load Local Environment With Flag
func LoadEnvironmentLocalFlag(file string) error {
	// load local env
	localEnv := viper.New()
	localEnv.SetConfigType("dotenv")
	localEnv.SetConfigFile(file)
...
func LoadEnvironmentParameter(defaultValues map[string]interface{}) {
	...
	if viper.GetString("ENVIRONMENT_SIMULATION") != "" {
		for k := range defaultValues {
