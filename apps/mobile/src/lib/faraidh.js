...
export function calculateFaraidh(input, total) {
  const {
    suami = 0, istri = 0, anakL = 0, anakP = 0,
    ayah = 0, ibu = 0, kakek = 0, nenek = 0,
    saudaraL = 0, saudaraP = 0,
  } = input;
  ...
  if (hasSon) {
    ...
    ashabah.push({ key: 'anak_laki', count: anakL, weight: (anakL * 2) / totalShares });
    if (anakP > 0) {
      ashabah.push({ key: 'anak_perempuan', count: anakP, weight: anakP / totalShares });
    }
  } else if (hasDaughter) {
    rows.push({ key: 'anak_perempuan', count: anakP, fraction: anakP === 1 ? fr(1, 2) : fr(2, 3) });
  }
  ...
  if (!hasChildren && !hasFather && !hasGrandfather) {
    if (isMusytarakah) {
      ...
      ashabah.push({ key: 'saudara_laki', count: saudaraL, weight: (saudaraL * 2) / totalShares });
      if (saudaraP > 0) {
        ashabah.push({ key: 'saudara_perempuan', count: saudaraP, weight: saudaraP / totalShares });
      }
    } else if (saudaraP > 0) {
      rows.push({ key: 'saudara_perempuan', count: saudaraP, fraction: saudaraP === 1 ? fr(1, 2) : fr(2, 3) });
    }
  }
  ...
  const finalRows = rows.map((r) => {
    ...
    return { key: r.key, count: r.count, fraction: r.fraction, share: dec, amount };
  });
  ...
  if (ashabah.length > 0) {
    ...
    ashabah.forEach((a) => {
      ...
      finalRows.push({ key: a.key, count: a.count, fraction: null, share, amount: (total ?? 0) * share, isAshabah: true });
    });
  }
