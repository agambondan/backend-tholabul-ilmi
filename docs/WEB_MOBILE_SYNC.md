...
| Perpustakaan Ilmu | `/library`, `/library/:slug` | `/dashboard/library`, `/dashboard/library/:slug` | Web and mobile both expose the same public catalog through `GET /api/v1/library/books`. Dashboard/detail journeys keep notes, bookmarks, study progress badges/filters, and resume surfaces inside the personal shell with `library_book` references and `/api/v1/library/progress`. |
